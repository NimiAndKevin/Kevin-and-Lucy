...
username=input("type your username to enter password type any username: ")
for i in range(3):
...
def listen_for_command():
    with sr.Microphone() as source:
        ...
        speak_properly(f"{username} i'm listening")
        print(f"Listening...")
...
def process_command(command):
    ...
    if ".com" in command or ".co.in" in command or ".org" in command:
        ...
        speak_properly(f"{username} i Opened the web you desired")
        webbrowser.open(f"https://www.{query}")
    elif "send message" in command or "text" in command:
        speak_properly("who do you want to message")
        name=input("Name: ")
        speak_properly("whats your message")
        message=input("message: ")
        speak_properly("when should i send the message")
        s_time=input("when should i send the message: ")
        if 'later' in s_time:
            hour_ = int(input("hour: "))
            minute_ = int(input("minutes: "))
        elif 'now' in s_time:
            hour_ = datetime.datetime.now().hour
            minute_ = datetime.datetime.now().minute
        speak_properly("sending message")
        kit.sendwhatmsg(name,message,hour_,minute_)
    ...
    elif "fullscreen" in command or "fullscreen video" in command:
        speak_properly(f"Setting it to fullscreen {username}")
        pyautogui.press('f')
    elif "escape" in command:
        speak_properly(f"escaping fullscreen {username}")
        pyautogui.press('Esc')
    ...
    elif "unmute" in command or "let me hear the video" in command:
        pyautogui.press("m")
        speak_properly(f"video unmuted {username}")
    elif "type" in command:
        speak_properly("{username} Please tell me what i should write")
        while True:
            typeQuery = command
        ...
        speak_properly("for how much seconds you want me to stop listening commands")
        a = int(listen_for_command())
        time.sleep(a)
        print(a)
        ...
        if conversion_result:
            ...
            speak_properly(f"{username} Video recording finished.")
    elif "stop recording" in command:
        cv2.destroyAllWindows()
        speak_properly(f"Closing Recording {username}")
    elif "i'm tired" in command or "i'm so tired" in command:
        speak_properly(f"let me Play your favourite songs, {username}")
        a = (1,2,3,4,5,6)
        b = random.choice(a)
        if b==1:
        ...
        if b==6:
            webbrowser.open("https://www.youtube.com/watch?v=Dxm3cHrKcbA")
        ...
        speak_properly(f"Opening Safari {username}.")
        open_safari() 
    elif "open app store" in command:
        speak_properly(f"Opening the App Store {username}.")
        open_app_store()
    elif "open settings" in command:
        speak_properly(f"Opening Settings {username}.")
        open_settings()
...
def introduce():
    speak_properly("I am Kevin, your virtual voice assistant made by Nimi my master. I can assist you with various tasks and answer your questions. Some of the things I can do include searching the web, opening websites, providing information, and much more.")
...
def WhatsappMsg(name,message):
    ...
    write(message)
    press('enter')
    speak_properly(f"sent whatsapp message to {name}")
...
if __name__ == "__main__":
    speak_properly(f"Hello {username} I'm Kevin how can i help you today")
    while True:
        command = listen_for_command()
        print(f"{username} said:", command)
